...
# get forms
CorpusDF$forma <- NA
CorpusDF$forma[!is.na(CorpusDF$sintagma)] <- CorpusDF$FORM[!is.na(CorpusDF$sintagma)]
...
ArgumentsDF <- CorpusDF[CorpusDF$DEPREL!='root',colnames(CorpusDF) %in% c('sintagma','forma','funcao','traco','tokenHEAD','sent_id')]
# load subset
...
# save RDS ##
saveRDS(VallencyDF,'./data/VallencyLT.rds')
