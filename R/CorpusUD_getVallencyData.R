...
ProielUDsentsList <- lapply(seq_along(dir(ProielUDconlluDir)), function(i) GetConlluMetadata(paste0(ProielUDconlluDir,dir(ProielUDconlluDir)[i]),'proiel_'))
...
VallencyDF <- VallencyDF[,which(str_detect(colnames(VallencyDF),'SENT_|HEAD_|ARG_'))]
## SAVE RDS ##
